...
def get_exp_name(flags):
    """Return a human-readable experiment name."""
    # Extract agent name from config
    agent_name = flags.agent.get('agent_name', 'unknown')

    # Extract key info
    env_short = flags.env_name.split('-')[0]  # e.g., 'cube' from 'cube-triple-play-...'
    seed = flags.seed
    tags = flags.tags if hasattr(flags, 'tags') else ''

    # Build readable name: {tags}_{agent}_{env}_{seed}
    if tags:
        exp_name = f"{tags}_{agent_name}_{env_short}_s{seed}"
    else:
        exp_name = f"{agent_name}_{env_short}_s{seed}"

    # Also compute hash for uniqueness (shorter version)
    full_flags = flags.flags_into_string()
    short_hash = get_hash(full_flags)[:8]

    exp_name = f"{exp_name}_{short_hash}"
